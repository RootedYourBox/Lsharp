...
namespace TongHop
{
	class TargetSelector
	{
		...
		{
			var temprange = LS.Player.AttackRange + LS.Player.BoundingRadius;
			foreach (
				var enemy in
					LS.AllHerosEnemy.Where(hero => hero.IsValidTarget(temprange + hero.BoundingRadius))
						.Where(enemy => LS.Player.GetAutoAttackDamage(enemy)*5 > enemy.Health))
				return enemy;

			Obj_AI_Hero[] tempTarget = {null};
			int[] tempPrio = {6};
			foreach(var enemy in LS.AllHerosEnemy.Where(hero => hero.IsValidTarget(temprange + hero.BoundingRadius)).Where(enemy => (tempTarget[0] == null) || (tempPrio[0] > GetPriorityForTarget(enemy.ChampionName)) || (tempPrio[0] == GetPriorityForTarget(enemy.ChampionName) && LS.Player.CalcDamage(enemy, Damage.DamageType.Physical, 100) > LS.Player.CalcDamage(tempTarget[0], Damage.DamageType.Physical, 100))))
			{
				tempTarget[0] = enemy;
		...
		private static Obj_AI_Hero AutoPrio(float range)
		{
			...
			foreach(var enemy in LS.AllHerosEnemy.Where(enemy => enemy.IsValidTarget(range + enemy.BoundingRadius)).Where(enemy =>
