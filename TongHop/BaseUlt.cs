...
namespace TongHop
{
	class BaseUlt
	{
		...
		private static void Game_OnGameUpdate(EventArgs args)
		{
			...
			if(!LS.Menu.Item("baseUlt").GetValue<bool>())
				return;
		...
		private static void HandleRecallShot(PlayerInfo playerInfo)
		{
			...
			foreach(var champ in _ownTeam.Where(x =>
				x.IsValid && (x.IsMe || Helper.GetSafeMenuItem<bool>(LS.Menu.Item(x.ChampionName))) &&
			...
			//use when fixed: champ.Spellbook.GetSpell(SpellSlot.R) = Ready or champ.Spellbook.GetSpell(SpellSlot.R).ManaCost)
			{
				...
				var timeneeded =
					...
					(LS.Menu.Item("extraDelay").GetValue<Slider>().Value + 65);
			...
			if(!shoot || LS.Menu.Item("panicKey").GetValue<KeyBind>().Active)
			{
				if(LS.Menu.Item("debugMode").GetValue<bool>())
			...
			if(time - playerInfo.LastSeen > 20000 && !LS.Menu.Item("regardlessKey").GetValue<KeyBind>().Active)
			{
		...
		{
			if(!LS.Menu.Item("showRecalls").GetValue<bool>())
				return;
			...
			foreach(var playerInfo in _playerInfo.Where(x =>
				...
				(x.Champ.IsEnemy || LS.Menu.Item("debugMode").GetValue<bool>())).OrderBy(x => x.GetRecallEnd()))
			{
				index++;
