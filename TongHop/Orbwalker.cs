...
namespace TongHop
{
	class Orbwalker
	{
		...
		private static readonly Obj_AI_Hero Player = LS.Player;
		...
		{
			if (LS.Menu.Item("orb_AutoWindup").GetValue<bool>())
			{
				var additional = 0;
				if (Game.Ping >= 100)
				...
				LS.Menu.Item("orb_ExtraWindup").SetValue(windUp < 200 ? new Slider(windUp, 200, 0) : new Slider(200, 200, 0));
			}
		...
		{
			if(target != null && CanAttack() && !LS.Menu.Item("orb_noattack").GetValue<bool>())
			{
			...
			if(!CanMove() || LS.Menu.Item("orb_nomove").GetValue<bool>())
				return;
			if(Player.IsMelee() && target != null && target.Distance(Player) < GetAutoAttackRangeto(target) &&
				LS.Menu.Item("orb_MeleePrediction").GetValue<bool>() && target is Obj_AI_Hero && Game.CursorPos.Distance(target.Position) < 300)
			{
		...
		private static Obj_AI_Base GetPossibleTarget()
		{
			...
			if(LS.Menu.Item("orb_Priority").GetValue<StringList>().SelectedIndex == 1 &&
		...
		public static Mode CurrentMode
		{
			get
			{
				if(LS.Menu.Item("orbkey_Combo").GetValue<KeyBind>().Active)
				{
					return Mode.Combo;
				}
				if(LS.Menu.Item("orbkey_Harass").GetValue<KeyBind>().Active)
				{
					return Mode.Harass;
				}
				if(LS.Menu.Item("orbkey_LaneClear").GetValue<KeyBind>().Active)
				{
					return Mode.LaneClear;
				}
				if(LS.Menu.Item("orbkey_Lasthit").GetValue<KeyBind>().Active)
				{
					return Mode.Lasthit;
				}
				return LS.Menu.Item("orbkey_Runlikehell").GetValue<KeyBind>().Active ? Mode.RunlikeHell : Mode.None;
			}
		}
		...
		private static int GetFarmDelay
		{
			get
			{
				return Player.ChampionName == "Azir"
					? LS.Menu.Item("orb_farmdelay").GetValue<Slider>().Value + 125 // with Azir Additional Delay
					: LS.Menu.Item("orb_farmdelay").GetValue<Slider>().Value;
			}
		}
		...
		{
			var extraWindup = LS.Menu.Item("orb_ExtraWindup").GetValue<Slider>().Value;
			if(_lastAATick <= Environment.TickCount)
